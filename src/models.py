...
def get_backbone(args):
    ...
    if args.model_config.backbone == 'vgg16':
        ...
        base_model = tf.keras.applications.InceptionResNetV2(include_top=False, weights=weights)
        base_model.trainable = True
        ...
        base_model = tf.keras.applications.EfficientNetV2B0(include_top=False, weights=weights)
        base_model.trainable = True
    elif args.model_config.backbone == 'EfficientNetV2M':
        base_model = tf.keras.applications.EfficientNetV2M(include_top=False, weights=weights)
        base_model.trainable = True
    else:
